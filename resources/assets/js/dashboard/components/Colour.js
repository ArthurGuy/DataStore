...
module.exports = {
    template: '<input type="range" name="{{name}}" value="{{hue}}" max="360" min="0" step="1" v-on="change: updateHue" style="padding: 5px 25px;">' +
    '<input type="range" value="{{saturation}}" max="100" min="0" step="1" v-on="change: updateSaturation" style="padding: 5px 25px;">' +
    '<div style="background-color:hsla({{hue}}, {{saturation}}%, 50%, 1); width:20px; height:20px;"></div>',
